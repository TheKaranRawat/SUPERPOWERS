...
local g_PersistWonderIcons = true;
...
function OnGenericWorldAnchor( type, bShow, iPlotX, iPlotY, iData1 )
    ...
    if( type == GenericWorldAnchorTypes.WORLD_ANCHOR_NATURAL_WONDER ) then
		if (g_PersistWonderIcons) then
			-- It's easier to treat the NW icon as a resource than to duplicate all the SV/Map and On/Off code here
			if (bShow) then
				LuaEvents.RIM_WonderAdded(iPlotX, iPlotY, iData1);
			end
		else
