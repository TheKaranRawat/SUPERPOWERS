...
local g_EarnedPromotionIM2   = InstanceManager:new( "EarnedPromotionInstance", "UnitPromotionImage", Controls.EarnedPromotionStack2 );
...
function UpdateUnitPortrait( unit )
	...
	if(unit:IsGreatPerson()) or unit:GetUnitCombatType() == GameInfoTypes.UNITCOMBAT_MAGICALGIRL then
		name = Locale.ConvertTextKey("TXT_KEY_CURRENTLY_SELECTED_UNIT");
    ...
    if textureOffset == nil then
        ...
        elseif (unit:GetNameNoDesc() == pPlayer:GetName()) then
                local leader = GameInfo.Leaders[pPlayer:GetLeaderType()];
                textureOffset, textureAtlas = IconLookup( leader.PortraitIndex, unitPortraitSize, leader.IconAtlas );
        --------------------------
        -- END OF MOD-MOD
        --------------------------
