...
function RefreshYourCulture()
	...
	for building in GameInfo.Buildings() do
		...
		if((buildingClass.MaxGlobalInstances > 0 and building.GreatWorkCount > 0) or
			building.Type == "BUILDING_HERMITAGE" or building.Type == "BUILDING_SAN_MARCO" or building.Type == "BUILDING_OXFORD_UNIVERSITY" or building.Type == "BUILDING_NATIONAL_THEATRE" or building.Type == "BUILDING_NATIONAL_LIBRARY" or building.Type == "BUILDING_NATIONAL_OPERA_HOUSE" or building.Type == "BUILDING_NATIONAL_LIBRARY" or building.Type == "BUILDING_NATIONAL_COLLEGE") then
	...
	for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS - 1 do	
		...
		if (pPlayer:IsAlive() and not pPlayer:IsMinorCiv() and activeTeam:IsHasMet(pPlayer:GetTeam())) then
			...
			if pPlayer ~= activePlayer then
				...
				local iImmigrationCounter = iHumanMoveToAI
				local AIMoveToHumanBase = activePlayer:GetInfluenceLevel(iPlayer)
				local HumanMoveToAIBase = pPlayer:GetInfluenceLevel(Game.GetActivePlayer())
				local strInternationalImmigrationToolTip = Locale.ConvertTextKey("TXT_KEY_CO_SP_MOVE_IN_BASE",AIMoveToHumanBase,HumanMoveToAIBase)
				...
				row.iExcessHappiness = iImmigrationCounter;
