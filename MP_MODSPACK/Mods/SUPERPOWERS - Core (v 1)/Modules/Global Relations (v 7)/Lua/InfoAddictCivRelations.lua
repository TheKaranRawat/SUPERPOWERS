...
local traderoute_color = violet;
local traderoute_text = violettext;
	...
	if (iPlayer ~= 0) then
		...
		if (not pActiveTeam:IsAtWar(pTeam)) then
			if approachID == MajorCivApproachTypes.MAJOR_CIV_APPROACH_HOSTILE then
				...
				statusTip	= Locale.ConvertTextKey("TXT_KEY_GR_HOSTILE")
			elseif approachID == MajorCivApproachTypes.MAJOR_CIV_APPROACH_GUARDED then
				...
				statusTip	= Locale.ConvertTextKey("TXT_KEY_GR_GUARDED")
			elseif approachID == MajorCivApproachTypes.MAJOR_CIV_APPROACH_AFRAID then
				statusIcon	= "[ICON_HAPPINESS_3]"
				statusColor	= "[COLOR_CULTURE_STORED]"
				statusTip	= Locale.ConvertTextKey("TXT_KEY_GR_AFRAID")
			elseif approachID == MajorCivApproachTypes.MAJOR_CIV_APPROACH_FRIENDLY then
				statusIcon	= "[ICON_HAPPINESS_1]"
				statusColor	= "[COLOR_GREEN]"
				statusTip	= Locale.ConvertTextKey("TXT_KEY_GR_FRIENDLY")
			else
				statusIcon	= "[ICON_HAPPINESS_2]"
				statusColor	= "[COLOR_WHITE]"
				statusTip	= Locale.ConvertTextKey("TXT_KEY_GR_NEUTRAL_STANCE")
			end
		else
			statusIcon	= "[ICON_WAR]"
			statusColor	= "[COLOR_RED]"
			statusTip	= Locale.ConvertTextKey("TXT_KEY_GR_AT_WAR")
		end
		str = str .. Locale.ConvertTextKey("TXT_KEY_GR_STANCE") .. " " .. statusColor  .. statusTip .. " " .. statusIcon;
	else
		str = str .. Locale.ConvertTextKey("TXT_KEY_GR_OUR_EMPIRE");
	end
	str = str .. "[NEWLINE]" .. "[COLOR_LIGHT_GREY]" .. Locale.ConvertTextKey("TXT_KEY_GR_SCORE") ..  " [COLOR_WHITE]" .. Players[iPlayer]:GetScore().. "[ENDCOLOR]";
	str = str .. "[NEWLINE]" .. "[COLOR_LIGHT_GREY]" .. Locale.ConvertTextKey("TXT_KEY_GR_ERA") ..  " [COLOR_WHITE]" .. Locale.ConvertTextKey(GameInfo.Eras[Players[iPlayer]:GetCurrentEra()].Description) .. "[ENDCOLOR]";
	if (not pActiveTeam:IsAtWar(pTeam)) then
		str = str .. "[NEWLINE]" .. "[COLOR_LIGHT_GREY]" .. Locale.ConvertTextKey("TXT_KEY_GR_INCOME") .. " [COLOR_WHITE]" .. Players[iPlayer]:GetGold() .. "/" .. Players[iPlayer]:CalculateGoldRate() .. "[ICON_GOLD][ENDCOLOR]";
	end
	if ( not (Game.GetActivePlayer() == iPlayer) ) then
		str = str .. "[NEWLINE]" .. "[COLOR_LIGHT_GREY]" .. Locale.ConvertTextKey("TXT_KEY_GR_MILITARY") .. " " .. getMilitaryPowerText(iPlayer) .. "[ENDCOLOR]";
	end

	-- Policies
	if ( not (Game.GetActivePlayer() == iPlayer) ) then
		local pOtherPlayer = Players[ iPlayer ];
		local strText = "[NEWLINE]" .. "[COLOR_CULTURE_STORED]" .. Locale.ConvertTextKey("TXT_KEY_GR_POLICY") .. "[ENDCOLOR]";
		for pPolicyBranch in GameInfo.PolicyBranchTypes() do
			...
			if (iCount > 0) then
				...
				strText = strText .. tooltipPad .. Locale.ConvertTextKey(pPolicyBranch.Description) .. ": " .. iCount;
...
function economicView()
  ...
  for thisPid = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
    if (hasMetCiv(thisPid) and Players[thisPid]:IsAlive()) then
	  ...
	  -- Any trade routes?
	  if (isBNW and traderoutesstr ~= "") then
        str = str .. "[NEWLINE]" .. traderoute_text .. Locale.ConvertTextKey("TXT_KEY_INFOADDICT_RELATIONTOOLTIP_TRADEROUTES") .. 
