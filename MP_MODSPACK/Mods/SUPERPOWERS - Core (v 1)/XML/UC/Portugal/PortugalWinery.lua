...
--Creates a Wine resource on a land plot that doesn't have a resource
				...
				if fPlot:GetFeatureType() == (-1) then
					if not fPlot:IsHills() then
						if fPlot:GetResourceType(-1) == (-1) then
							fPlot:SetResourceType(GameInfoTypes.RESOURCE_WINE, 1)
							return true
						end
					end
				end
...
--Watch to see when a Winery is constructed
	if building == GameInfoTypes.BUILDING_PORTUGAL_WINERY then
