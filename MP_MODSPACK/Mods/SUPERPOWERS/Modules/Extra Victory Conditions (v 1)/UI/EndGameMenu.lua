...
function OnDisplay( type, team )
	...
	if( team == Game.GetActiveTeam() ) then
		...
		-- EVC Start
		if (g_EVCReligiousVictoryWon == true) then
			Controls.EndGameText:SetText( Locale.ConvertTextKey( "TXT_KEY_VICTORY_FLAVOR_RELIGIOUS" ) );
	    	victoryType = "VICTORY_RELIGIOUS";
		elseif (g_EVCEconomicVicotryWon == true) then
			Controls.EndGameText:SetText( Locale.ConvertTextKey( "TXT_KEY_VICTORY_FLAVOR_ECONOMIC" ) );
	    	victoryType = "VICTORY_ECONOMIC";
		end
		-- EVC End
