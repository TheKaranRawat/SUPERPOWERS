...
function ShowAirliftRangeIndicator()
	...
	if pHeadSelectedUnit:CanAirlift(thisPlot, false) then		
		for iPlotLoop = 0, Map.GetNumPlots()-1, 1 do
			...
			---------------SP fix airlift to enemy's city bug (temp method) ----------------
			
			local iPlayerID = Game.GetActivePlayer()
			local plotOwnerID = pPlot:GetOwner()
			
			if pHeadSelectedUnit:CanAirliftAt(thisPlot, plotX, plotY) and plotOwnerID == iPlayerID then
			
			---------------SP fix airlift to enemy's city bug (temp method) END ----------------	
...
ContextPtr:LoadNewContext("GlobalRazeCityStates")
...
ContextPtr:LoadNewContext("NewCityRule")
ContextPtr:LoadNewContext("NewCombatRules")
ContextPtr:LoadNewContext("NewHandicap")
ContextPtr:LoadNewContext("NewPolicyEffects")
ContextPtr:LoadNewContext("NewPopulationRule")
ContextPtr:LoadNewContext("NewTraitEffects")
ContextPtr:LoadNewContext("NewUnitsRules")
ContextPtr:LoadNewContext("GAIA_NotifyNuclearAttack")
ContextPtr:LoadNewContext("NuclearWinter")
...
ContextPtr:LoadNewContext("UINotifyExitResistance")
